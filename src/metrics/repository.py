...
async def add_commit_metric(data: CommitMetrics, db: AsyncSession = Depends(get_db)) -> int:
...
async def get_metrics_by_commit_id(commit_id: int, db: AsyncSession = Depends(get_db)) -> CommitMetrics:
...
async def delete_metric_by_commit_id(commit_id: int, db: AsyncSession = Depends(get_db)) -> int:
