...
async def get_commits_by_user_id(user_id: int, db: Session = Depends(get_db)) -> List[UserCommits]:
    query = select(UserCommits).where(UserCommits.user_id == user_id)
    result = await db.execute(query)
    commit_models = result.scalars().all()
    return commit_models

async def delete_commit_by_id(commit_id: int, db: Session = Depends(get_db)) -> int:
